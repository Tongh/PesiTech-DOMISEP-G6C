...
class PieceModel extends Model {
  ...
  function viewLogement() {
    $id_logement = $_GET["id_logement"];
		$sql = 'SELECT * FROM `piece` WHERE 	`id_logement` = \'' . $id_logement . '\'';
		if ($result = $this -> query($sql)) {
			return $result;
    ...
    $id_logement = $_SESSION["id_logement"];
		...
		if ($result = $this -> query($sql)) {
			...
			$sql = 'INSERT INTO `piece` (`superficie`, `nom`, `type`, `id_logement`, `id_client`)  VALUES 	(\'' . $superficie . '\', \'' . $nom . '\', \'' . $type . '\', \'' . $id_logement . '\', \'' . $id_login .'\')';
